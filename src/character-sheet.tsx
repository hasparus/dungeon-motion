...
const controlStyle =
  "px-2 pt-1 pb-0.5 text-[10px] leading-none [text-box-trim:both] [text-box-edge:cap_alphabetic] font-medium rounded border border-stone-400 dark:border-stone-500 text-stone-500 dark:text-stone-400 hover:text-stone-700 dark:hover:text-stone-300 hover:border-stone-500 dark:hover:border-stone-400 transition-colors bg-transparent";

function SheetControls() {
  const [names, setNames] = useState<string[]>(() => Object.keys(getSheets()));

  return (
    <div className="fixed top-3 right-3 print:hidden flex gap-1.5 z-50">
      <button
      ...
      <select
        className={cn(controlStyle, "cursor-pointer")}
        onChange={(e) => {
  ...
  return (
    <div>
      <p className="font-hand text-base text-stone-500 dark:text-stone-400 mb-3">
        <span className="font-serif font-bold tracking-wider text-stone-700 dark:text-stone-300">
          Stats
        </span>{" "}
...
function CombatShapes() {
  return (
    <div className="flex justify-center items-end gap-8">
      ...
      >
        {/* Heart — QuiverAI Arrow (inner outline only, stroke mode) */}
        <path
      ...
      >
        {/* Shield — QuiverAI Arrow inner path as stroke outline */}
        <path
      ...
      <ShapedStat label="Damage" name="damage" viewBox="30 0 196 256">
        {/* Die (d4) — QuiverAI Arrow, inner outline only, stroke mode */}
        <path
...
function InlineField({
  ...
  return (
    <label className="flex items-baseline gap-2 flex-1">
      <span
        className={cn(
          "font-bold tracking-wide text-stone-700 dark:text-stone-300 shrink-0 whitespace-nowrap",
          large ? "text-base" : "text-sm translate-y-0.5",
        )}
      >
        {label}
      </span>
      <input
        className={cn(
          "flex-1 min-w-0 bg-transparent border-b border-stone-300 dark:border-stone-600 print:border-stone-400 leading-none",
